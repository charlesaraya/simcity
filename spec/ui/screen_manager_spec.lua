...
local function screen_stub()
    ...
    s.mousemoved     = record("mousemoved")
...
describe("ScreenManager.register / set_current", function()
    ...
    it("clear_current resets current_id to nil (used when entering in-game)", function()
        local mgr = ScreenManager.new()
        mgr:register("home", screen_stub())
        mgr:set_current("home")
        mgr:clear_current()
...
describe("ScreenManager modal stack", function()
    ...
    it("clear_modals empties the stack regardless of depth", function()
        local mgr = ScreenManager.new()
        mgr:register("a", screen_stub())
        mgr:register("b", screen_stub())
        mgr:push_modal("a")
        mgr:push_modal("b")
        mgr:push_modal("a")
        mgr:clear_modals()
    ...
    it("clear_modals on an empty stack is a no-op", function()
        local mgr = ScreenManager.new()
        assert.has_no.errors(function() mgr:clear_modals() end)
        assert.are.equal(0, mgr:modal_count())
    end)
...
describe("ScreenManager dispatch (update / input)", function()
    ...
    it("routes mousemoved to the top active surface", function()
        ...
        assert.are.same({ 5, 7 }, home.hits[1].args)
        mgr:push_modal("pause")
        mgr:mousemoved(8, 9)
        assert.are.equal(1, #home.hits) -- unchanged
        assert.are.equal(1, #pause.hits)
        assert.are.same({ 8, 9 }, pause.hits[1].args)
    end)
