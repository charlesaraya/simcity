...
function Power.compute_topology(world)
    ...
    -- Label every conducting tile with its component (the shared NetworkedUtility
    -- flood-fill); power's own layer is the per-component supply below.
    local component, count = Network.components(grid, is_conductor)
    local supply = {}
    for cid = 1, count do supply[cid] = 0 end
...
-- (the shared NetworkedUtility query over the powered set).
function Power.building_powered(world, x, y)
    return Network.adjacent(world.grid, world.power.powered or {}, x, y)
end
